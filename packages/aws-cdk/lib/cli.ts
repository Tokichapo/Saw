...
async function parseCommandLineArguments(args: string[]) {
  ...
  return yargs
    ...
    .command('diff [STACKS..]', 'Compares the specified stack with the deployed stack or a local template file, and returns with status 1 if any difference is found', (yargs: Argv) => yargs
      ...
      .option('change-set', { type: 'boolean', alias: 'changeset', desc: 'Whether to create a changeset to analyze resource replacements. In this mode, diff will use the deploy role instead of the lookup role.' }))
    ...
    switch (command) {
      ...
      case 'diff':
        ...
        return cli.diff({
          ...
          quiet: args.quiet,
          changeSet: args['change-set'] ?? configuration.settings.get(['diff', 'changeSet']) ?? true,
        });
